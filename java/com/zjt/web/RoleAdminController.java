...
public class RoleAdminController extends BaseContrller{
        ...
        try {
            ...
            if (trole.getId() != null && !trole.getId().equals(0)) {
                ...
                if (role == null) {
                    ...
                    //还需删除用户角色中间表，角色的id存在于两个中间表中所有要删除两个中间表中的信息
...
System.out.println(getUser().getUserName()+"张现增");
            resultmap.put("state", "success");
...
//根据id查询某个角色的信息
    ...
    //如果想要单独测试那么就把下面的注释掉这样就不交给shiro管理了
    @RequiresPermissions(value = {"角色管理"})
     ...
     * @throws Exception
     */
    //根据页面上角色表后面的设置菜单权限来查看都具有那些可以附加给该角色的菜单
    @ResponseBody
